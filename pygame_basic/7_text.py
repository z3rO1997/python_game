...
character_x_pos = screen_width / 2  - character_width / 2# 화면 가로 절반 크기에 해당하는 위치 (가로)
...
enemy_x_pos = (screen_width / 2)  - (enemy_width / 2)# 화면 가로 절반 크기에 해당하는 위치 (가로)
...
while running:
    dt = clock.tick(144) # 게임 화면 초당 프레임 수 설정
    ...
    # 경과 시간 (ms)을 1000을 나누어 초(s) 단위로 표시

    timer = game_font.render(str(int(total_time - elapsed_time)), True, (255, 255, 255))# 출력할 글자, antielapse True, 글자 색상
